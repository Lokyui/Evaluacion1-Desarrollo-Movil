...
Porque lo hice asi
Me cambié a Expo Router porque facilita mucho la navegación basada en archivos, parecido a Next.js si vienes de web.

Para las fotos y ubicación, usé las librerias nativas de Expo porque son las que mejor funcionan y son fáciles de integrar.

Decidí separar la lógica de guardado (storage) de la vista para mantener el código más limpio. Así si mañana quiero cambiar AsyncStorage por una base de datos real, solo toco ese archivo.

Problemas que tuve
Al principio me costó configurar los permisos de la cámara y ubicación en el archivo app.json, a veces la app se cerraba si no los pedía bien.
También tuve que pelear un poco con el layout de la cámara para que se viera bien y no tapara los botones.
La navegación con Expo Router es genial pero tuve que aprender cómo pasar parámetros y cómo proteger rutas (el login).
